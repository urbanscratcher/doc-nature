...
- 여러 구현방법이 존재함
- 객체 지향적 관점에서 1번 방법은 사용하지 않는 것이 좋음
- 2, 3번은 관계를 어떤 관점에서 보느냐에 따라 명칭이 달라짐
- 따라서 4번이 가장 적절
...
- 거리(distance)가 0이면 분모가 될 수 없음
- 또한 지나치게 떨어져 있을 경우 힘이 너무 약하게 적용될 것
- 두 물체 사이의 거리가 5픽셀 이하 혹은 25 픽셀 이상이라면 제한하는 코드 추가
...
- 위 코드에서 Mover, Attractor의 지름은 각 물체의 질량에 따라 조정됨
- 원의 면적: $\pi r^2$
- 면적에 따른 질량을 더 정확히 표현하려면 실제 질량의 제곱근을 취하고, 이를 원의 지름으로 조정함

:::info[연습문제 2.11]
예제 코드 2.6에서 Attract, Mover의 질량을 면적에 대응시키기
...
- Attractor 객체가 화면에 보이지 않게 하고, Mover 객체가 움직이는 궤적을 시각화 해 보기. 그러면 클레이튼 커빗, 톰 카든이 만든 메트로팝 데님 프로젝트와 같은 모습을 볼 수 있음.

:::info[연습문제 2.12]
예제 코드 2.7에는 Mover 배열과 하나의 Attractor 객체가 존재함. Attractor도 다수인 경우를 구현해보기. 또한 Attractor가 보이지 않게 만들면 어떻게 될까? Attractor 주위를 움직이는 물체의 흔적에서 패턴을 만들 수 있을까?
:::

:::info[연습문제 2.13]
가까우면 약해지고, 멀면 강해지는 힘을 구현해보고, 척력(repulsion force)도 적용해보기. 척력이란 인력과 다르게 밀어내는 힘을 의미. 가능한 한 많은 힘을 생각해서 구현해보기.
:::
